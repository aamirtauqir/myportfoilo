      ...
      if (VLTJS.html.hasClass("vlt-is--homepage")) {
        VLTJS.window.on("vlt.change-slide", function () {
          animatedBlock.each(function () {
            ...
            $this
              .removeClass(prefix + "animated")
              .removeClass(prefix + animationName);
            if ($this.parents(".vlt-section").hasClass("active")) {
              $this
                .addClass(prefix + "animated")
                .addClass(prefix + animationName);
            }
          });
        });
      } else {
    ...
    init: function () {
      $(".vlt-content-slider").each(function () {
        var $this = $(this),
          ...
          scrollbar_settings = $this.data("scrollbar-settings")
            ? (scrollbar_settings = JSON.stringify(
                $this.data("scrollbar-settings")
              ))
            : false;

        $this.data("scrollbar-settings")
          ? ((scrollbar_obj.el = $this.data("scrollbar-settings").target),
            scrollbar_array.push(scrollbar_obj))
          : false;

        console.log(scrollbar_settings);
        console.log(scrollbar_obj);
        ...
        var swiper = new Swiper(container, {
          ...
          breakpoints: {
            ...
            768: {
              slidesPerView:
                settings.slides_to_show_tablet || settings.slides_to_show || 1,
              slidesPerGroup:
                settings.slides_to_scroll_tablet ||
                settings.slides_to_scroll ||
                1,
        ...
        swiper.on("init slideChange", function () {
          ...
          gsap.fromTo(
            ...
            { opacity: 1, scale: 1, duration: 0.5, duration: 1 }
          );
        });

        swiper.on("slideChangeTransitionStart", () => {
          if ($(this).find(".ourwork-slide").length) {
            gsap.fromTo(
...
(function ($) {
  ...
  VLTJS.fullpageSlider = {
    init: function () {
      ...
      $(
        ".vlt-offcanvas-menu ul.sf-menu > li:first-of-type, .vlt-default-menu__navigation ul.sf-menu > li:first-of-type"
      ).addClass("active");
      ...
      function gsap_animation_scroll(index, nextIndex, direction) {
        const currentSection = $(".vlt-fullpage-slider .pp-scrollable")
          .eq(index)
          .data("anchor");

        if (currentSection == "Partners") {
          gsap.from(".vlt-partners li", {
            duration: 1.5,
            y: 100,
            opacity: 0,
            rotation: 0,
            stagger: {
              each: 0.3,
              from: "start", // Staggering starts from the center
            },
            ease: "elastic.out(1, 0.3)", // Elastic easing for a bouncy effect
          });
        }

        if (currentSection == "OurWork") {
          ...
          //     end: () =>
          //       "+=" + document.querySelector(".gsapcontainer").offsetWidth,
          //   },
          // });
        }
      }
      ...
      fullpageSlider.pagepiling({
        ...
        onLeave: function (index, nextIndex, direction) {
          vlthemes_navigation();
          VLTJS.window.trigger("vlt.change-slide");
          gsap_animation_scroll(index, nextIndex, direction);
        },
        afterLoad: function () {
          // console.log(anchors);
          vlthemes_navbar_solid();
